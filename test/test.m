...
a=0.40;  % minor radius
e=a/R0;
rhoh=0.0; % vh/Omega=sqrt(2Th/M)/Omega, Omega=Be/M
...
nt=102; % theta grid number for poloidal angle
    ...
    for j2=1:ne+1
        for j1=1:nt+1
            %for j=1:nx+1
                ...
                Gfun4D(j3,j2,j1,:)=Gfun(gtt2D(j1,:),grt2D(j1,:),...
                    grr2D(j1,:),kappa_t2D(j1,:),kappa_r2D(j1,:),...
                    fxi_t1D,fxi_r1D);
            %end
        end
    end
    ...
    for j2=1:ne+1
        for j1=1:nt+1
            %for j=1:nx+1
                fxi_r1D(:)=xi_r4D(j3,j2,j1,:);
                fxi_t1D(:)=xi_t4D(j3,j2,j1,:);
                GLbfun4D(j3,j2,j1,:)=GLbfun(gtt2D(j1,:),grt2D(j1,:),grr2D(j1,:),...
